...
PathTwoD = os.path.abspath(os.path.join(os.path.dirname( __file__ ), '../..'))
TwoD = imp.load_source('QuadLagrangeGaussLobatto',PathTwoD+'/Core/InterpolationFunctions/TwoDimensional/Quad/QuadLagrangeGaussLobatto.py')
PathThreeD = os.path.abspath(os.path.join(os.path.dirname( __file__ ), '../..'))
ThreeD = imp.load_source('HexLagrangeGaussLobatto',PathThreeD+'/Core/InterpolationFunctions/ThreeDimensional/Hexahedral/HexLagrangeGaussLobatto.py')
				...
				if np.count_nonzero(t)!=0:
					...
					for i in range(0,w.shape[0]):
						for j in range(0,w.shape[0]):
							...
							# Unit normal
							unit_normal = np.cross(J[:,face_index[0]],J[:,face_index[1]])/np.linalg.norm(np.cross(J[:,face_index[0]],J[:,face_index[1]]))
...
def ApplyNeumannBoundaryConditions(C,mesh,nmesh,BoundaryData,nvar,Basis,w,z,elem):
	...
	mesh_edges = np.array(mesh.edges)		# Get free edges (edges at the boundary) - Check if this is available for all meshes used
	...
	if np.array(edge_nodes).shape[0]!=0:
		...
		if edge_nodes.shape[0]==2:
			...
			# Romove duplicates
			# Since at the edge corner of a 2D mesh one is common and after sort that node is going to comprise one column of 'edge' here - print edge to check (right here)
			# Carefull
		...
		for iedge in range(0,edge.shape[0]):
			...
			if np.count_nonzero(force)!=0:
				...
				if C>0:
					local_element_nodes = np.linspace((C+1)*local_element_nodes[0],(C+1)*local_element_nodes[1],(C+1)*(local_element_nodes[1]-local_element_nodes[0])+1)
				for g in range(0,w.shape[0]):
					for k in range(0,nvar):
