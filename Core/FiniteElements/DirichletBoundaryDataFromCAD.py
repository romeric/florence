...
def PostMeshWrapper(MainData,mesh):
    ...
    if MainData.ndim == 2:
        ...
        if planar_mesh_faces.shape[0] != 0:
            ...
            GetDirichletDataForPlanarFaces(MainData,mesh,planar_mesh_faces,nodesDBC,Dirichlet,plot=False)
...
def GetDirichletDataForPlanarFaces(MainData,mesh,planar_mesh_faces,nodesDBC,Dirichlet,plot=False):
    ...
    for niter in range(number_of_planar_surfaces):
        ...
        if pmesh.points.shape[0] != Dirichlet2D.shape[0]:
            ...
            TotalDisp = MainSolver(MainData2D,pmesh)
        else:
