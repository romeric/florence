...
def Assembly(MainData,mesh,Eulerx,TotalPot):
    ...
    if mesh.nelem <= 500000:
        return AssemblySmall(MainData,mesh,Eulerx,TotalPot)
    elif mesh.nelem > 500000:
        print("Larger than memory system. Dask on disk parallel assembly is turned on")
        return OutofCoreAssembly(MainData,mesh,Eulerx,TotalPot)
    # else:
...
def AssemblySmall(MainData,mesh,Eulerx,TotalPot):
    ...
    if MainData.Parallel:
        ...
        ParallelTuple = parmap.map(GetElementalMatricesSmall,np.arange(0,nelem,dtype=np.int32),
            MainData,mesh.elements,mesh.points,Eulerx,TotalPot,
...
def OutofCoreAssembly(MainData, mesh, Eulerx, TotalPot, calculate_rhs=True, filename=None, chunk_size=None):
