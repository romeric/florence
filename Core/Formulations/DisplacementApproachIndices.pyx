...
cdef inline void GetTotalTraction_(double *CauchyStressTensor,double *TotalTraction, int ndim):
	if ndim == 2:
...
def GetTotalTraction(np.ndarray[double,ndim=2,mode='c'] CauchyStressTensor):
	...
	GetTotalTraction_(&CauchyStressTensor[0,0],&TotalTraction[0,0],ndim)
