...
# @profile
def HighOrderMeshTet_SEMISTABLE(C,mesh,Decimals=10,Zerofy=0,Parallel=False,nCPU=1,ComputeAll=False):
    ...
    # FIND DUPLICATES IN FLOATING POINT ARRAY REPOINTS
    duplicates = get_duplicates(repoints,Decimals,Parallel=False)
    tdups = time() - tnodes
    ...
    ParallelTuple3=[]
    if Parallel:
        ParallelTuple3 = parmap.map(ElementLoopTet_2,np.arange(mesh.points.shape[0],mapnodes.shape[0]),
            reelements,remainingnodes,pool=MP.Pool(processes=nCPU))
    ...
    if ComputeAll == True:
        ...
        for iface in range(0,mesh.faces.shape[0]):
            if Parallel:
                ...
                D = mesh.points[mesh.faces[iface,2],:]
                # FIND THE VECTORS OF ALL EDGES
                BA = B - A      # VECTOR OF EDGE 1
                DA = D - A      # VECTOR OF EDGE 2
                DB = D - B      # VECTOR OF EDGE 3

                EA = repoints[mesh.points.shape[0]:,:] - np.repeat(A.reshape(1,3),repoints.shape[0] - mesh.points.shape[0],axis=0 )
                EB = repoints[mesh.points.shape[0]:,:] - np.repeat(B.reshape(1,3),repoints.shape[0] - mesh.points.shape[0],axis=0 )

                # COMPUTE AREAS 
                Area = 0.5*np.linalg.norm(np.cross(BA,DA)); Area = Area*np.ones(EA.shape[0])
                # Area1 = 0.5*np.linalg.norm(np.cross(DA,BA),axis=1); #Area1 = Area1*np.ones(EA.shape[0])
                Area1 = 0.5*np.linalg.norm(np.cross(EA, np.repeat(BA.reshape(1,3),EA.shape[0],axis=0)),axis=1 )
                Area2 = 0.5*np.linalg.norm(np.cross(EA, np.repeat(DA.reshape(1,3),EA.shape[0],axis=0)),axis=1 )
                Area3 = 0.5*np.linalg.norm(np.cross(EB, np.repeat(DB.reshape(1,3),EB.shape[0],axis=0)),axis=1 )
                # t1 +=( time()-tt)
                # CHECK THE CONDITION
                # j = np.where(np.abs( Area - (Area1+Area2+Area3) )  < ftol)[0]
                j = np.asarray(whereLT1d(np.abs( Area - (Area1+Area2+Area3) ),ftol)) 
                # j = np.asarray(whereLT(np.abs( Area - (Area1+Area2+Area3) ).reshape(Area.shape[0],1),ftol)[0]) 
            if j.shape[0]==refaces.shape[1]-3:
                refaces[iface,3:] = j+mesh.points.shape[0]
            # else:
            #     warn('More nodes within the tetrahedral face than necessary. \
            #             This could be due to either high or low tolerance')
        ...
        # NEW STABLE APPROACH - CORE BIT / UNCOMMENT TO RUN
        ##############################################################################################
        # tt = time()
        ...
        # print 'unstable old time\t', tfaces
        # print 'stable new time  \t', tt

        # print np.sort(refaces,axis=1) - np.sort(refaces2,axis=1)
        # print np.sum(np.sort(refaces,axis=1) - np.sort(refaces2,axis=1))
