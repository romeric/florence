...
class FEMSolver(object):
    ...
    def __init__(self,
        ...
        activate_line_search=False,
        activate_arc_length=False,
        ...
        self.activate_line_search = activate_line_search
        self.activate_arc_length = activate_arc_length
        ...
        self.has_computed_ideal_elements = False
        self.ideal_elements = []
    ...
    def __makeoutput__(self, mesh, TotalDisp, formulation=None, function_spaces=None, material=None):
        ...
        if self.compute_mesh_qualities and self.is_scaled_jacobian_computed is False:
            ...
            post_process.ScaledFF, post_process.ScaledHH, post_process.ScaledJacobianF, post_process.ScaledJacobian \
                = post_process.MeshQualityMeasures(mesh,TotalDisp,False,False)[1:5]
        elif self.is_scaled_jacobian_computed:
            post_process.ScaledJacobian=self.ScaledJacobian
            post_process.ScaledJacobianF=self.ScaledJacobianF
    ...
    def Solve(self, formulation=None, mesh=None,
        ...
        if self.analysis_type == "static":
            # self.sol = boundary_condition.UpdateFixDoFs(boundary_condition.applied_dirichlet,
        ...
        for Increment in range(LoadIncrement):
            ...
            if Increment == LoadIncrement - 1:
                ...
                if self.compute_mesh_qualities:
                    ...
                    self.ScaledJacobianF = qualities[3]
                    self.ScaledJacobian = qualities[4]
    ...
    def StaticSolver(self, function_spaces, formulation, solver, K,
        ...
        for Increment in range(LoadIncrement):
            ...
            elif self.nonlinear_iterative_technique == "quasi_newton_raphson":
                Eulerx, Eulerp, K, Residual = self.QuasiNewtonRaphson(function_spaces, formulation, solver,
                    Increment, K, NodalForces, Residual, mesh, Eulerx, Eulerp,
                    material, boundary_condition, AppliedDirichletInc)
            elif self.nonlinear_iterative_technique == "lbfgs":
                Eulerx, Eulerp, K, Residual = self.LBFGS(function_spaces, formulation, solver,
                    Increment, K, NodalForces, Residual, mesh, Eulerx, Eulerp,
                    material, boundary_condition, AppliedDirichletInc)
            else:
                raise RuntimeError("Iterative technique for nonlinear solver not understood")
    ...
    def NewtonRaphson(self, function_spaces, formulation, solver,
        ...
        while self.norm_residual > Tolerance or Iter==0:
            ...
            if Iter==0:
                ...
                if np.isclose(self.NormForces, 0.):
                    self.NormForces = 1e-14
    ...
    def LBFGS(self, function_spaces, formulation, solver,
        ...
        while self.norm_residual > Tolerance or Iter==0:
            ...
            if Iter == 0:
                ...
                F_b = boundary_condition.GetReducedMatrices(K,Residual,only_residual=True)
            ...
            if Iter > 0:
                sk = alpha * dU - dU0
                yk = Residual - R0
                rhok = 1. / (np.einsum("i,i",sk.flatten(),yk.flatten()) + 1e-14)
    ...
    def GoldsteinLineSearch(self, function_space, formulation, mesh, material,
        ...
        ResidualCopy[boundary_condition.columns_in] = TractionForces[boundary_condition.columns_in] -\
            NodalForces[boundary_condition.columns_in]
        R0 = np.dot(dU[:,:formulation.ndim].flatten().T,ResidualCopy).item()

        # RE-ASSEMBLE - COMPUTE INTERNAL TRACTION FORCES
        TractionForces = AssembleInternalTractionForces(self, function_space, formulation, mesh, material,
            newXCopy + dU[:,:formulation.ndim], newPCopy + dU[:,-1])
        # FIND THE RESIDUAL
        ResidualCopy[boundary_condition.columns_in] = TractionForces[boundary_condition.columns_in] -\
            NodalForces[boundary_condition.columns_in]
        ...
        while np.abs(Rf) > rho * np.abs(R0) and Iter < 25:
            ...
            if False:
                too_small = True
                break
            # print(alpha, np.abs(Rf) , rho * np.abs(R0))
    ...
    def WolfeLineSearch(self, function_space, formulation, mesh, material,
        ...
        while EXf > EX0 + c * alpha * pkX and np.linalg.norm(np.dot(dU.flatten().T,ResidualCopy).item()) > c2 * np.linalg.norm(pkX):
            ...
            # print(alpha, np.linalg.norm(np.dot(dU.flatten().T,ResidualCopy).item()), c2 * np.linalg.norm(pkX))
