...
def GetDirichletData(mesh,nurbs,boundary_condition,BasesOrder):
    ...
    for kFace in range(mesh.edges.shape[0]):
        ...
        for iBC in range(1):
            ...
            if np.all(boundary_condition.nurbs_condition[kFace]) == True:
            # if np.all(boundary_condition.SetNURBSCondition(NURBSCondition,edge_coords)):
