...
template<typename U>
class _IsotropicElectroMechanics_109_: public _MaterialBase_<U> {
public:
    U mu_v;
    U mu1;
    U mu2;
    ...
    _IsotropicElectroMechanics_109_() = default;

    FASTOR_INLINE
    _IsotropicElectroMechanics_109_(U mu_v, U mu1, U mu2, U lamb, U eps_2) {
        this->mu_v = mu_v;
        this->mu1 = mu1;
        this->mu2 = mu2;
        this->lamb = lamb;
    ...
    FASTOR_INLINE
    void SetParameters(U mu_v, U mu1, U mu2, U lamb, U eps_2){
        this->mu_v = mu_v;
        this->mu1 = mu1;
        this->mu2 = mu2;
        this->lamb = lamb;
    ...
    _KineticMeasures_(const T *Fnp, const T *Enp) {
        ...
        Tensor<T,ndim,ndim> sigma_linear = 2.*mu_v*strain + lamb*tre*I;

        Tensor<T,ndim,ndim> sigma = sigma_mech + sigma_electric + sigma_linear;
        ...
        Tensor<T,ndim,ndim,ndim,ndim> C_elect = 1./eps_2*(0.5*innerDD*( II_ijkl + II_ikjl + II_iljk) - \
                    IDD_ijkl - DDI_ijkl );
        Tensor<T,ndim,ndim,ndim,ndim> C_linear = mu_v*(II_ikjl + II_iljk) + lamb*II_ijkl;
        Tensor<T,ndim,ndim,ndim,ndim> elasticity = C_mech + C_elect + C_linear;
        ...
        Tensor<T,ndim,ndim> dielectric = 1./eps_2*I;
...
template<> template<>
void _IsotropicElectroMechanics_109_<Real>::KineticMeasures<Real>(Real *Dnp, Real *Snp, Real* Hnp,
