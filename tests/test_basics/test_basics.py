...
from Florence.MeshGeneration.CustomMesher import HarvesterPatch
from Florence.MeshGeneration.CustomMesher import SubdivisionArc, SubdivisionCircle, QuadBall, QuadBallSphericalArc
...
def test_mesh_postprocess_material():
    ...
    for etype in etypes:
        ...
        pp.GetAugmentedSolution()
        pp.SetSolution(np.zeros_like(mesh.points))
        pp.SetMaterial(PixarNeoHookean(2,mu=1.,lamb=10.))
        pp.GetAugmentedSolution()
        pp.SetSolution(np.zeros_like(mesh.points))
    ...
    for etype in etypes:
        ...
        mesh.FaceNormals()
        mesh.Volumes()
        mesh.Volumes(with_sign=True)
        mesh.Sizes()
        mesh.Sizes(with_sign=True)
        mesh.Lengths()
        mesh.EdgeLengths()
        mesh.AspectRatios()
        mesh.Medians()
        ...
        pp.GetAugmentedSolution()
        pp.SetSolution(np.zeros_like(mesh.points))
        pp.SetMaterial(PixarNeoHookean(3,mu=1.,lamb=10.))
        pp.GetAugmentedSolution()
        pp.SetSolution(np.zeros_like(mesh.points))
    ...
    for n in range(11):
        mesh = QuadBall(n=n)
        mesh = QuadBallSphericalArc(n=n)
    mesh = QuadBallSphericalArc()
    mesh = QuadBallSphericalArc(portion=0.25)
    mesh = QuadBallSphericalArc(portion=0.5)
